...
plot(best_output.t_vec, best_output.y_smooth); hold off
...
% simulate
function [t_vec, theta, theta_hat, y] = simulate(a, b, k, w, ph, pl, dt, W1, W2, y0, du0, t0, tf, func)
    % setup
    delay = 0;
    t_vec = 0:dt:tf;
    N = length(t_vec) - 1;
    ...
    plant_buffer = zeros(delay+1,1) + y0; % initialize to y0
    ...
    for i = 2:N+1
        ...
        du0 = y0 - get_y0(theta(max(i-1-delay,1)));
        t0 = t_vec(i); % new time

        % add noise
        % y0 = y0 + 0.05*(rand(1)-0.5);
    end
