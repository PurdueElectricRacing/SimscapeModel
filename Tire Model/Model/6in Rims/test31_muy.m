...
z_muy_31 = zeros(num_entries, 7);
counter = 1;

for j = 1:num_entries
    if abs(IA(j)) < 0.5
        z_muy_31(counter, 1) = FZ(j);
        z_muy_31(counter, 2) = SL(j);
        z_muy_31(counter, 3) = abs(FY(j) / FZ(j));
        z_muy_31(counter, 4) = SA(j);
        z_muy_31(counter, 5) = IA(j);
        z_muy_31(counter, 6) = abs(FY(j));
        z_muy_31(counter, 7) = abs(FX(j));
        counter = counter + 1;
    end
end

z_muy_31( all(~z_muy_31,2), : ) = [];


%% Sort By FZ & Peak FY
% FZ categories
target_value = [204 429 669 896 1125];
tolerance = target_value.*0.025;

% Peak FY
peaks = [670 1250, 1820, 2310 2660];
dip_from_peak = [70, 150, 170, 140 201];

% Initialize variables
num_data = length(z_muy_31);
...
for i = 1:5
    counter = 1;
    for j = 1:num_data
        if abs(abs(z_muy_31(j, 1)) - target_value(i)) < tolerance(i)
            if abs(abs(z_muy_31(j, 6)) - peaks(i)) < dip_from_peak(i)
                FZ_mux(counter, 1, i) = z_muy_31(j, 3);
                FZ_mux(counter, 2, i) = z_muy_31(j, 6);
                FZ_mux(counter, 3, i) = z_muy_31(j, 1);
                FZ_mux(counter, 4, i) = z_muy_31(j, 2);
                FZ_mux(counter, 5, i) = z_muy_31(j, 4);
                FZ_mux(counter, 6, i) = z_muy_31(j, 5);
                FZ_mux(counter, 7, i) = z_muy_31(j, 7);
                counter = counter + 1;
            end
        end
    end
