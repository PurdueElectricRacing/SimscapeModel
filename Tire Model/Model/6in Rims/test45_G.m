...
z_G_45 = zeros(num_entries, 6);
counter = 1;

for j = 1:num_entries
    if abs(SA(j)) < 0.5 && abs(SL(j)) < 0.05
        z_G_45(counter, 1) = abs(FZ(j));
        z_G_45(counter, 2) = SL(j);
        z_G_45(counter, 3) = SA(j);
        z_G_45(counter, 4) = IA(j) .* (pi / 180);
        z_G_45(counter, 5) = FY(j);
        z_G_45(counter, 6) = FX(j);
        counter = counter + 1;
    end
end
...
for i = 1:4
    counter = 1;
    for j = 1:num_data
        if abs(abs(z_G_45(j, 1)) - target_value(i)) < tolerance(i)
            FZ_mux(counter, 1, i) = z_G_45(j, 1);
            FZ_mux(counter, 2, i) = z_G_45(j, 2);
            FZ_mux(counter, 3, i) = z_G_45(j, 3);
            FZ_mux(counter, 4, i) = z_G_45(j, 4);
            FZ_mux(counter, 5, i) = z_G_45(j, 5);
            FZ_mux(counter, 6, i) = z_G_45(j, 6);
            counter = counter + 1;
        end
    end
end
