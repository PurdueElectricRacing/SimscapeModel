...
z_C_31 = zeros(num_entries, 6);
counter = 1;

for j = 1:num_entries
    if abs(IA(j)) < 0.5 && abs(SA(j)) < 1
        z_C_31(counter, 1) = abs(FZ(j));
        z_C_31(counter, 2) = SL(j);
        z_C_31(counter, 3) = abs(SA(j) .* (pi / 180));
        z_C_31(counter, 4) = IA(j);
        z_C_31(counter, 5) = abs(FY(j));
        z_C_31(counter, 6) = abs(FX(j));
        counter = counter + 1;
    end
end
...
for i = 1:5
    counter = 1;
    for j = 1:num_data
        if abs(abs(z_C_31(j, 1)) - target_value(i)) < tolerance(i)
            FZ_mux(counter, 1, i) = z_C_31(j, 1);
            FZ_mux(counter, 2, i) = z_C_31(j, 2);
            FZ_mux(counter, 3, i) = z_C_31(j, 3);
            FZ_mux(counter, 4, i) = z_C_31(j, 4);
            FZ_mux(counter, 5, i) = z_C_31(j, 5);
            FZ_mux(counter, 6, i) = z_C_31(j, 6);
            counter = counter + 1;
        end
    end
end
...
% Establish Boundary Conditions on Function
FZ_y(1, 6) = 0;
FY_SA(1, 6) = 0;

FZ_y(1, 7) = FZ_y(1, 5) + 200;
FY_SA(1, 7) = FY_SA(1, 5);

% compute C = f(FZ)
C = pchip(FZ_y, FY_SA, abs(FZ));
