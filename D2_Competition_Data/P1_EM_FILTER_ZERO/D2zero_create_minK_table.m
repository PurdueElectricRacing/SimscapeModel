...
function [v_sweep,w_sweep,minK_table,voltages,max_rpm] = D2zero_create_minK_table()
%% Parameters
...
for i = 1:1:num_datasets
    ...
    motor_constants(:,i) = table2array(readtable("D2zero_data/D2zero_all_motor_data.xlsx", opts, "UseExcel", false));
end

motor_constants(1,:) = motor_constants(1,:)*rpm2radps;
motor_constants(4,:) = motor_constants(4,:)*rpm2radps;

max_rpm = motor_constants(1,:) - motor_constants(2,:).*motor_constants(4,:);

%% Generate Table of min k
% create input data (rotation speed, voltage)
w_sweep = linspace(0, max(max_rpm), RPM_resolution);  % motor shaft speed (rad/s)
...
WM = (ones(num_datasets,1)*max_rpm);

w_table = (WM./max_rpm'<=1).*WM;
v_table = VM'.*(WM./max_rpm'<=1);
