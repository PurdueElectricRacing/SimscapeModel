...
function r_TVS = compute_r_TVS(inp, sys_bias, sys_gain, add_gain, var, r_EQUAL, lo_constant, ang_vel,vel_gs, phi, dphi, TVS_I, TVS_P)
    ...
    inp = [inp(1)+sys_bias(1), inp(2)+sys_bias(2), inp(3)+sys_bias(3), inp(4)+sys_bias(4)];
    ...
    min_saturation_out = min(max(min_out, var.rb(1)), var.rb(2))
    ...
    yaw_lookup = var.yaw_table(vel_gs, 3);
