              ...
              yVCU_struct *y)
{
  ...
  if (y->VCU_mode == 4.0F) {
    ...
    if (y->VT_mode == 1.0F) {
      ...
      float Xqk;
      float Yqk;
      Xqk = fmaxf(fminf(y->ST_CF, p->TV_ST_ub), p->TV_ST_lb);
      Yqk = fmaxf(fminf(y->GS_CF, p->TV_GS_ub), p->TV_GS_lb);
      if ((Xqk >= p->TV_ST_brkpt[0]) && (Xqk <= p->TV_ST_brkpt[52]) &&
        ...
        if (Xqk == p->TV_ST_brkpt[b_low_i]) {
          ...
          if (Xqk == f8) {
            ...
            int b_qx1_tmp_tmp;
            int qx1_tmp_tmp;
            rx = (Xqk - p->TV_ST_brkpt[b_low_i]) /
                 (f8 - p->TV_ST_brkpt[b_low_i]);
        ...
        f9 = p->TV_GS_brkpt[c_low_i - 1];
        if ((Yqk == f9) || (qx1 == qx2)) {
          y->TV_AV_ref = qx1;
        } else if (Yqk == p->TV_GS_brkpt[c_low_i]) {
          y->TV_AV_ref = qx2;
        } else {
          float ry;
