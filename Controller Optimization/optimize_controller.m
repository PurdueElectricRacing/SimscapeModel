...
for i = 1:num1
    for j = 1:num2
        ...
        for k = 1:num3
            ...
            exit_flag(i,j,k) = sum(0>exit_flag_all);

            Tx_all = outputs.get('Tx').Values.Data;
            Tx_test(i,j,k) = max(Tx_all(617:end,1)) - min(Tx_all(617:end,1));

            yaw_accel_error_all = outputs.get('Tracking Signal').Values.Data;
            yaw_accel_error(i,j,k) = sqrt(mean((yaw_accel_error_all).^2));
            ...
            yaw_error(i,j,k) = sqrt(mean((yaw_error_all).^2));  
            toc
